  ...
  return (
    <div className="flex flex-col">
      {activities.map((a, i) => (
        <div
          key={a.id}
          className={`flex items-center gap-3 py-3 ${i < activities.length - 1 ? 'border-b border-gray-100' : ''}`}
        >
          ...
          <div className="text-right shrink-0">
            {a.training_stress_score ? (
              <div className="text-sm font-bold text-orange-600 tabular-nums">
                ...
                <span className="text-gray-400 text-xs font-normal ml-0.5">TSS</span>
              </div>
            ) : null}
            {a.avg_heart_rate ? (
              <div className="text-xs text-gray-400 tabular-nums">{Math.round(a.avg_heart_rate)} bpm</div>
            ) : null}
          </div>
        </div>
      ))}
