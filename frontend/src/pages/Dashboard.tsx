  ...
  return (
    <div className="flex items-center gap-2.5 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5">
      <div className={clsx('w-7 h-7 rounded-lg flex items-center justify-center shrink-0',
        good === true ? 'bg-emerald-50' : good === false ? 'bg-rose-50' : 'bg-gray-100')}>
        <Icon size={13} className={good === true ? 'text-emerald-600' : good === false ? 'text-rose-500' : 'text-gray-500'} />
      </div>
...
function LoadMetric({
  ...
  const trendColor = trend === 'up' ? 'text-emerald-500' : trend === 'down' ? 'text-rose-400' : 'text-gray-400'
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <p className="text-4xl font-black text-gray-900 tabular-nums">{Math.round(value)}</p>
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex items-center justify-between">
        ...
        {fitness && (
          <div className="flex items-center gap-2 bg-white border border-gray-100 rounded-full px-3 py-1.5 text-xs shadow-sm">
            <span className="text-gray-500">{fitness.season === 'SKI' ? '⛷️' : '🚴'}</span>
      ...
      {readiness && (
        <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-4">Today's Readiness</p>
          <div className="flex gap-6 items-start">
            <ReadinessRing score={readiness.score} zone={readiness.zone} />
            <div className="flex-1 flex flex-col gap-3">
              <p className="text-sm text-gray-600 leading-relaxed">{readiness.guidance}</p>
      ...
      <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-4">Training Load</p>
      ...
      {fitness && fitness.series.length > 0 && (
        <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
          <h2 className="text-sm font-semibold text-gray-700 mb-5">90-Day Training Load</h2>
      ...
      <div>
        ...
        {plan ? (
          ...
          <div className="bg-white rounded-xl p-8 border border-gray-100 shadow-sm text-center">
            <p className="text-gray-500 text-sm mb-3">No plan for this week yet.</p>
            <a
              href="/plan"
        ...
        {plan?.narrative && (
          <div className="mt-3 bg-white rounded-xl p-4 border border-orange-100 shadow-sm">
            <p className="text-xs text-orange-600 font-semibold uppercase tracking-widest mb-1.5">Coach's Note</p>
            <p className="text-sm text-gray-600 leading-relaxed">{plan.narrative}</p>
          </div>
        )}
      </div>
